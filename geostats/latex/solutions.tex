...
\noindent where \texttt{T} is the `time base', corresponding to the
number of years. $H_0$ is rejected on a 95\% confidence level, and so
it would be best to cease operations and abandon the mining
site. Instead of using the number of diamonds per tonne to define
$\lambda$, we can also use the number of diamonds per 10 tonnes:

  $H_0: \lambda=20$\\
  $H_{\!A}: \lambda<20$

\noindent which allows us the calculate a rejection region:
...
\item First, we take the mean of the data:
  ...
  with 4 degrees of freedom. Using $\alpha=0.05$ and
...
\item We are comparing the following hypotheses:
  ...
  distribution at $\alpha=0.025$.  Using Table~\ref{tab:w0.025}, the
  rejection region is $R=\{w<{38}\}$. $46\notin{R}$, so we cannot
...
\includegraphics[]{../figures/PCAiris.pdf}\medskip
\end{minipage}
...
\noindent\begin{minipage}[t][][b]{.35\linewidth}
\includegraphics[]{../figures/elbow.pdf}\medskip
\end{minipage}
\begin{minipage}[t][][t]{.65\linewidth}
...
\includegraphics[]{../figures/testPCA.pdf}\medskip
\end{minipage}
...
Estimating the log[Zn] values along an $(x,y)$-grid:
...
colourplot(x=xi,y=yi,z=zsph,plot.title=title('spherical'))
colourplot(x=xi,y=yi,z=zexp,plot.title=title('exponential'))
\end{script}
...
colourplot(X=X,Y=Y,x=xi,y=yi,z=exp(zsph-zexp),colspec=grey.colors,
           key.title=title(expression('Zn'[sph]*'/Zn'[exp]),xpd=NA))
\end{script}

\noindent which exponentiates the log-contrasts to yield the ratio of
the Zn-concentations estimated using the spherical and exponential
semivariogram models.

\noindent\begin{minipage}[t][][b]{.6\linewidth}
\includegraphics[]{../figures/sphvsexp.pdf}
\end{minipage}
